...
func (s *bootstrapSuite) TestBootstrapNeedsSettings(c *gc.C) {
	...
	err := bootstrap.Bootstrap(envtesting.BootstrapContext(c), env, bootstrap.BootstrapParams{ControllerUUID: coretesting.ModelTag.Id()})
...
func (s *bootstrapSuite) TestBootstrapEmptyConstraints(c *gc.C) {
	...
	c.Assert(env.args, gc.DeepEquals, environs.BootstrapParams{ControllerUUID: coretesting.ModelTag.Id()})
}

func (s *bootstrapSuite) TestBootstrapMismatchedControllerUUIDs(c *gc.C) {
	env := newEnviron("foo", useDefaultKeys, nil)
	s.setDummyStorage(c, env)
	err := bootstrap.Bootstrap(envtesting.BootstrapContext(c), env, bootstrap.BootstrapParams{ControllerUUID: "foo"})
	c.Assert(err, gc.ErrorMatches, "controller config UUID does not match specified controller UUID")
}
...
func (s *bootstrapSuite) TestBootstrapGUISuccessLocal(c *gc.C) {
	...
	env := newEnviron("foo", useDefaultKeys, nil)
	ctx := coretesting.Context(c)
	err := bootstrap.Bootstrap(modelcmd.BootstrapContext(ctx), env, bootstrap.BootstrapParams{ControllerUUID: coretesting.ModelTag.Id()})
	c.Assert(err, jc.ErrorIsNil)
